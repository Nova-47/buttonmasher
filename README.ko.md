...
대부분의 코드 리뷰는 당신의 코드를 읽습니다. 이건 실제 사용자처럼 코드를 실행합니다 — Submit 더블클릭, 타임아웃 후 재시도, 뒤로가기 후 다시, 탭 두 개, 웹훅 재전송.

그럴 때 두 가지가 깨집니다. **정확히 한 번만 일어나야 할 일이 두 번 일어납니다** — 결제, 주문, 예약, 재고 차감, 쿠폰 사용, 크레딧 차감, 이메일. 그리고 **이미 끝난 일이 다시 일어납니다** — 취소된 예약이 재조정되고, 만료된 토큰이 여전히 먹히고, 완료된 작업이 다시 돕니다.

그 버그들은 코드에 드러나지 않습니다. 순서에 드러납니다.

> 코드 리뷰의 대체재가 아닙니다. 우리 자체 벤치마크에서, 작은 템플릿 레포에서는 평범한 리뷰 프롬프트가 우리보다 실제 버그를 더 많이 찾았습니다(10 대 5). 상태 있는 프로덕션 경로에서는, 그 프롬프트가 0건 찾은 라이프사이클·멱등성 버그를 우리가 6건 찾았습니다. 두 실행 다 [`bench/`](bench/)에 원시 출력까지 있습니다.
