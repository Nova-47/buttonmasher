...
activate가 아무것도 확인하지 않는다: verified_at도, plan도.
북마크한 URL 하나로 인증 안 된 계정이 plan = nil인 채 활성화된다.
...
세션 안에서 띄울 수 있을 만큼 가벼운 레포 — 라이브 DB나 무거운 빌드 없이 설치·실행되는 레포 — 면 앱을 띄워 요청을 실제로 두 번 보냅니다.

띄울 수 없는 무거운 레포면 코드 경로를 손으로 추적하고, 각 발견을 `Reproduced`가 아니라 `Traced`로 표시해 어느 쪽인지 항상 알 수 있게 합니다.

## 심각도

| 라벨 | 의미 |
|---|---|
| **BROKE** | 돈이 잘못 움직이거나, 레코드가 중복·유실되거나, 잘못된 상태가 저장됐다. |
| **FRAGILE** | 여기선 정상이지만, 현실적인 레이스·재시도면 BROKE가 된다. |
...
**두 번 돌렸더니 결과가 다른데요. 어느 쪽이 맞나요?**

둘 다 맞습니다. 그때그때 눈에 띄는 가장 날카로운 다섯 개를 고르는데, 매번 같은 다섯 개를 보는 건 아닙니다. 각 발견은 `Reproduced`(실행함)/`Traced`(코드 추적)로도 표시됩니다. 심각도는 결과 기준이지 레포를 띄웠는지와 무관합니다. 합집합이 필요하면 두 번 돌리세요.
